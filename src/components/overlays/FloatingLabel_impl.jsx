// ── overlays/FloatingLabel_impl.jsx ──────────────────────────────────────
...
//                  subtitle (white), danger (red)
//   start_ms     — when the label appears
//   end_ms       — when it disappears (optional, stays for rest of scene)
//   anchor_word  — STT word to anchor timing to (overrides start_ms if set)
...
const STYLES = {
  label: {
    background: 'rgba(27,42,74,0.88)', color: '#FFFFFF',
    fontSize: 28, fontWeight: 700, padding: '8px 18px', borderRadius: 6,
  },
  highlight: {
    background: 'rgba(201,168,76,0.92)', color: '#1B2A4A',
    fontSize: 32, fontWeight: 700, padding: '8px 20px', borderRadius: 6,
  },
  subtitle: {
    background: 'transparent', color: '#FFFFFF',
    fontSize: 24, fontWeight: 400,
    textShadow: '0 2px 8px rgba(0,0,0,0.85)', padding: '4px 12px',
  },
  danger: {
    background: 'rgba(153,27,27,0.88)', color: '#FFFFFF',
    fontSize: 28, fontWeight: 700, padding: '8px 18px', borderRadius: 6,
  },
};
...
export const TextElement = ({
  ...
  // Resolve start from anchor_word if provided
  ...
  const scale      = interpolate(localFrame, [0, 8], [0.85, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });

  const pos      = POSITIONS[position] || POSITIONS.top_left;
  const styleObj = STYLES[style]       || STYLES.label;

  return (
    <div style={{ position: 'absolute', ...pos, zIndex: 10, opacity,
      transform: `${pos.transform || ''} scale(${scale})` }}>
      <span style={{ display: 'inline-block', fontFamily: brand?.font_heading || 'Oswald', ...styleObj }}>
        {text}
      </span>
    </div>
  );
